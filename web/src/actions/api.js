 ...
 */
export function initiateApiRequest(endpoint, options = {}) {
  return { type: API_REQUEST_INITIATED, endpoint, options };
}
...
export function getFromApi(path) {
  return initiateApiRequest(`${ROWBOAT_API_URI}${path}`);
}
 ...
 */
export function postToApi(api, path, data) {
  const options = {
    method: 'POST',
  ...
  return initiateApiRequest(`${ROWBOAT_API_URI}${path}`, options);
}
 ...
 * @param  {Object} data Data to attach as the request payload.
 */
export function putToApi(api, path, data) {
  const options = {
    method: 'PUT',
    body: JSON.stringify(data),
    headers: new Headers({
